...
一个 [Service](/docs/concepts/services-networking/service/)，一个
[PodDisruptionBudget](/docs/concepts/workloads/pods/disruptions//#specifying-a-poddisruptionbudget)，
和一个 [StatefulSet](/docs/concepts/workloads/controllers/statefulset/)。

{{< codenew file="application/zookeeper/zookeeper.yaml" >}}

打开一个命令行终端，使用
[`kubectl apply`](/docs/reference/generated/kubectl/kubectl-commands/#apply) 创
建这个清单。
...
使用 [`kubectl get`](/docs/user-guide/kubectl/{{< param "version" >}}/#get) 查看
StatefulSet 控制器创建的 Pods。
...
使用 [`kubectl exec`](/docs/user-guide/kubectl/{{< param "version" >}}/#exec) 获
取 `zk` StatefulSet 中 Pods 的主机名。
...
[Kubernetes DNS](/docs/concepts/services-networking/dns-pod-service/) 中的 A 记
录将 FQDNs 解析成为 Pods 的 IP 地址。如果 Pods 被调度，这个 A 记录将会使用 Pods
的新 IP 地址更新，但 A 记录的名称不会改变。
...
最基本的健康检查是向一个 ZooKeeper 服务写入一些数据，然后从另一个服务读取这些数
据。
...
使用 [`kubectl
delete`](/docs/user-guide/kubectl/{{< param "version" >}}/#delete) 删除 `zk`
StatefulSet。
...
StatefulSet 的容器 `template` 中的 `volumeMounts` 一节使得 PersistentVolumes 被
...
使用 [`kubectl logs`](/docs/user-guide/kubectl/{{< param "version" >}}/#logs) 从
一个 Pod 中取回最后几行日志。
...
特权用户运行，你可以使用
[SecurityContext](/docs/tasks/configure-pod-container/security-context/) 控制运
行容器入口点的用户。
...
在 Pods 容器内部，UID 1000 对应用户 zookeeper，GID 1000 对应用户组 zookeeper。
...
[Restart Policies](/docs/user-guide/pod-states/#restartpolicy) 控制 Kubernetes
...
这个探针调用一个简单的 bash 脚本，使用 ZooKeeper 的四字缩写 `ruok` 来测试服务的
健康状态。
...
虽然存活性探针和可读性探针是相同的，但同时指定它们两者仍然重要。这保证了
...
上一小节展示了如何在节点之间分散 Pods 以在计划外的节点故障时存活。但是你也需要为
计划内维护引起的临时节点故障做准备。
...
使用 [`kubectl
cordon`](/docs/user-guide/kubectl/{{< param "version" >}}/#cordon) cordon 你的集
群中除 4 个节点以外的所有节点。
...
使用 [`kubectl drain`](/docs/user-guide/kubectl/{{< param "version" >}}/#drain)
来 cordon 和 drain `zk-0` Pod 调度的节点。
...
使用 [`kubectl
uncordon`](/docs/user-guide/kubectl/{{< param "version" >}}/#uncordon) 来取消对
第一个节点的隔离。
