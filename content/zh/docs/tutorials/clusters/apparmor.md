...
Kubernetes AppArmor 强制首先通过检查所有先决条件都已满足，然后将配置文件选择转发
到容器运行时进行强制。如果未满足先决条件， Pod 将被拒绝，并且不会运行。
...
- 通过在每个节点上运行 Pod
  的[DaemonSet](/docs/concepts/workloads/controllers/daemonset/)确保加载了正确的
  配置文件。可以找到一个示例实
...
用[节点选择器](/docs/concepts/configuration/assign pod node/)确保 Pod 在具有所需
配置文件的节点上运行。
