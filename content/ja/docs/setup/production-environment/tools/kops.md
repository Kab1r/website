...
- Multiple OS support (Debian, Ubuntu 16.04 supported, CentOS & RHEL, Amazon
  ...
  [images.md](https://github.com/kubernetes/kops/blob/master/docs/images.md)
- High-Availability support - see the
  [high_availability.md](https://github.com/kubernetes/kops/blob/master/docs/high_availability.md)
...
If your opinions differ from these you may prefer to build your own cluster
using [kubeadm](/docs/admin/kubeadm/) as a building block. kops builds on the
kubeadm work.
...
You must have [kubectl](/ja/docs/tasks/tools/install-kubectl/) installed in
order for kops to work.
...
### (2/5) クラスタ用の route53 ドメインの作成

kops uses DNS for discovery, both inside the cluster and so that you can reach
the kubernetes API server from clients.
...
- Slack Channel:
  [#kops-users](https://kubernetes.slack.com/messages/kops-users/)
...
- See the `kops` [docs](https://github.com/kubernetes/kops) section for
