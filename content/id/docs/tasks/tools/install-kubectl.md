...
barisan perintah Kubernetes yang digunakan untuk menjalankan berbagai perintah
...
1. Jalankan perintah berikut (jangan lupa untuk memasukkan `DownloadLocation`):
   ...
   {{< note >}}Jika kamu tidak menambahkan `DownloadLocation`, `kubectl` akan
...
[Chocolatey](https://chocolatey.org) atau _installer_ barisan perintah
...
1. Lakukan konfigurasi `kubectl` agar menggunakan _remote_ kluster Kubernetes:
   ...
   {{< note >}}Ubah berkas konfigurasi dengan editor teks pilihanmu, misal
   Notepad.{{< /note >}}
...
[berkas kubeconfig](/docs/tasks/access-application-cluster/configure-access-multiple-clusters/),
...
Jika `kubectl cluster-info` mengembalikan respons URL tetapi kamu masih belum
...
menambahkan skrip tersebut ke dalam berkas `~/.bashrc`, tergantung paket manajer
yang kamu pakai.

Untuk memastikan, muat ulang terminalmu dan jalankan `type _init_completion`.
Jika perintah berhasil maka instalasi selesai. Jika tidak, tambahkan teks
berikut ke dalam berkas `~/.bashrc`:
...
Jika versinya sudah terlalu usang, kamu dapat menginstal/memperbaruinya dengan
...
{{< note >}} bash-completion v2 yang diinstal dengan Homebrew meletakkan semua
berkas nya di direktori `BASH_COMPLETION_COMPAT_DIR`, yang membuat dua cara
...
Tambahkan baris berikut di berkas `~/.zshrc` untuk menyalakan _auto complete_
dari `kubectl`:
