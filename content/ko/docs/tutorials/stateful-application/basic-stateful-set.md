...
헤드리스 서비스의 CNAME은 SRV 레코드를 지칭한다 (Running과 Ready 상태의 각 파드
...
파드의 순번, 호스트네임, SRV 레코드와 A 레코드이름은 변경되지 않지만파드의 IP 주
...
다른 터미널창에서 `kubectl scale`을 이용하여 레플리카 개수를 5로 스케일링하자.
...
[스케일 다운](#scaling-down) 섹션에서 보았듯 파드는각 순번의 역순으로 하나씩 종
