...
참고하길 바란다. 파드시큐리티폴리시 인증에 대한 전체 예제는 [아래](#예제)를 참�
하길 바란다.
...
_이 예에서는 파드시큐리티폴리시 어드미션 컨트롤러가 활성화된 클러스터가 실행 중
...
- _MayRunAs_ - 하나 이상의 `range`를 지정해야 한다. 기본값을 제공하지 않�
...
**ReadOnlyRootFilesystem** - 컨테이너는 읽기-전용 루트 파일시스템(즉, 쓰기 가능
한 레이어 없음)으로실행해야 한다.
...
`allowedProcMountTypes`는 허용된 ProcMountTypes의 화이트리스트이다. 비어 있거나
...
파드에서 seccomp 프로파일의 사용은 파드시큐리티폴리시의 어노테이션을 통해제어�
수 있다. Seccomp는 쿠버네티스의 알파 기능이다.

**seccomp.security.alpha.kubernetes.io/defaultProfileName** - 컨테이너에적용�
