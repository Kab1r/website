...
. 이러한 레플리카는 대체 가능하다. 즉, 프론트엔드는 그것들이 사용하는 백엔드를�
...
가지로, 서비스 정의를 API 서버에 `POST`하여새 인스턴스를 생성할 수 있다.
...
파드의 포트 정의에는 이름이 있고, 서비스의 `targetPort` 속성에서 이 이름을참조�
...
서비스의 기본 프로토콜은 TCP이다. 다른 [지원되는 프로토콜](#protocol-support)을
사용할 수도 있다.

많은 서비스가 하나 이상의 포트를 노출해야 하기 때문에, 쿠버네티스는 서비스 오브
젝트에서 다중포트 정의를 지원한다. 각 포트는 동일한 `프로토콜` 또는 다른 프로�
콜로 정의될 수 있다.
...
예를 들어, `123-abc` 와 `web` 은 유효하지만, `123_abc` 와 `-web` 은 유효하지 않
다. {{< /note >}}
...
예를 들면, 쿠버네티스 네임스페이스 `"my-ns"`에 `"my-service"`라는서비스가 있는
...
- `ClusterIP`: 서비스를 클러스터-내부 IP에 노출시킨다. 이 값을 선택하면클러스터
...
- [`NodePort`](#nodeport): 고정 포트 (`NodePort`)로 각 노드의 IP에 서비스를노출
  ...
  `<NodeIP>:<NodePort>`를 요청하여, 클러스터 외부에서 `NodePort` 서비스에 접속�
  수 있다.
...
`service.beta.kubernetes.io/aws-load-balancer-access-log-s3-bucket-name` 어노테
...
가있는 경우, 비(non)-NLB 트래픽도 해당 수정된 보안 그룹의모든 인스턴스에 도달�
수 있다.
...
ExternalName 유형의 서비스는 `my-service` 또는 `cassandra`와 같은 일반적인 셀렉
...
컨트롤 플레인에서, 백그라운드 컨트롤러는 해당 맵을생성해야 한다. (인-메모리 잠금
...
향후, 서비스에 대한 프록시 정책은 예를 들어, 마스터-선택 또는 샤드 같은단순한 라
운드-로빈 밸런싱보다 미묘한 차이가 생길 수 있다. 또한일부 서비스에는 "실제" 로드
밸런서가 있을 것으로 예상되는데, 이 경우가상 IP 주소는 단순히 패킷을 그곳으로 전
송한다.

쿠버네티스 프로젝트는 L7 (HTTP) 서비스에 대한 지원을 개선하려고 한다.

쿠버네티스 프로젝트는 현재 ClusterIP, NodePort 및 LoadBalancer 모드 등을 포함하
는 서비스에 대해보다 유연한 인그레스 모드를 지원하려고 한다.
